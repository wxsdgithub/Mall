...
<script type="text/javascript">
	...
	function mNum(gid,p,no){
		var num = -1; //数量
		//下面语句的语法就是为了获得id为（num_count连上no）的值，这里其实就是购物车页面商品的当前数量值
		//判断该数量就是为了判读当数量小于1是是否要继续，因为这就意味着把该商品全部删除了
		var nums = $("#num_count"+no).val();
		//验证是否需要删除，另外Number(nums)等价于nums
		if(Number(nums)<=1){
			if(confirm("确认要删除吗?")){
				/* num = 0; */
				//当最后一件商品删除完，就是意味着该商品全部删除
	...
	<div class="row" style="margin-top: 40px;">
		<div class="col-md-10 col-md-offset-1">
			<table class="table table-bordered table-striped table-hover">
 				...
 				<c:forEach items="${userCart}" var="c" varStatus="i">
	 				<tr>
	 					<th>${i.count}</th>
	 					<th>${c.name}</th>
	 					<th>
							<img src="./fmwimages/${c.picture}" width="50px" height="50px">
						</th>
	 					<th>${c.price}</th>
	 					<th width="100px">
